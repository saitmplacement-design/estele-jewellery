...
(function () {
  ...
  /* Floating Cart Function - MINIMAL ADDITION */
  function updateFloatingCart(count) {
    var floatingCart = document.getElementById('floating-cart');
    var chatWidget = document.querySelector('[data-chat]');
    var floatingCartBadge = document.querySelector('[data-floating-cart-count]');
    ...
    if (count > 0) {
      ...
      if (floatingCartBadge) {
        ...
        floatingCartBadge.hidden = false;
      }
      // Move chatbot up slightly to avoid overlap
      if (chatWidget) {
        chatWidget.style.bottom = '88px';
      }
    } else {
  ...
  /* Update existing cart functions to call updateFloatingCart */
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Override add to cart buttons if they exist
    var addButtons = document.querySelectorAll('[data-add-to-cart]');
    addButtons.forEach(function(btn) {
      btn.addEventListener('click', function() {
        setTimeout(function() {
          var newCount = 0;
          try { newCount = parseInt(localStorage.getItem('estele-cart-count'), 10) || 0; } catch (e) {}
          updateFloatingCart(newCount);
        }, 100);
      });
    });
  });